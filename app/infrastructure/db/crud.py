...
from app.infrastructure.security import DuplicateUserError
from app.infrastructure.db.database import execute, get_conn, get_last_id
...
# ── Agent Run CRUD ──
# agent_run 表存储任务推进状态（目标、计划、进度、阻塞、验收、重规划）。
    ...
    try:
        ...
        execute(cursor,
            ...
            INSERT INTO agent_run (
                user_id, session_id, goal, goal_type, status,
                plan_json, current_step,
                completed_steps_json, pending_steps_json, failed_steps_json,
                blockers_json, next_action,
                acceptance_criteria_json, verification_results_json,
                final_report, next_suggestions_json, task_ids_json
            ) VALUES (?, ?, ?, ?, 'created', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                user_id,
                ...
                goal,
                goal_type,
                json.dumps(plan_steps, ensure_ascii=False) if plan_steps is not None else None,
                current_step,
                json.dumps(completed_steps, ensure_ascii=False) if completed_steps is not None else None,
                json.dumps(pending_steps, ensure_ascii=False) if pending_steps is not None else None,
                json.dumps(failed_steps, ensure_ascii=False) if failed_steps is not None else None,
                json.dumps(blockers, ensure_ascii=False) if blockers is not None else None,
                next_action,
                json.dumps(acceptance_criteria, ensure_ascii=False) if acceptance_criteria is not None else None,
                json.dumps(verification_results, ensure_ascii=False) if verification_results is not None else None,
                final_report,
                json.dumps(next_suggestions, ensure_ascii=False) if next_suggestions is not None else None,
                json.dumps(task_ids, ensure_ascii=False) if task_ids is not None else None,
            ),
        )
        conn.commit()
        run_id = int(get_last_id(cursor))
        return get_agent_run(run_id, user_id=user_id) or {}
    except Exception as exc:
        if conn is not None:
            conn.rollback()
        raise RuntimeError("Failed to create agent run") from exc
    finally:
        if conn is not None:
            conn.close()
...
def get_agent_run(
    ...
    try:
        ...
        execute(cursor,
            """
            SELECT id, user_id, session_id, goal, goal_type, status,
                   plan_json, current_step,
                   completed_steps_json, pending_steps_json, failed_steps_json,
                   blockers_json, next_action,
                   acceptance_criteria_json, verification_results_json,
                   replan_count, final_report, next_suggestions_json, task_ids_json,
                   created_at, updated_at
            FROM agent_run
            WHERE id = ? AND user_id = ?
            """,
        ...
        run = _row_to_dict(cursor.fetchone())
        if run is None:
            return None
        run["plan_json"] = _loads_json_or_raw(run["plan_json"])
        run["completed_steps_json"] = _loads_json_or_raw(run["completed_steps_json"])
        run["pending_steps_json"] = _loads_json_or_raw(run["pending_steps_json"])
        run["failed_steps_json"] = _loads_json_or_raw(run["failed_steps_json"])
        run["blockers_json"] = _loads_json_or_raw(run["blockers_json"])
        run["acceptance_criteria_json"] = _loads_json_or_raw(run["acceptance_criteria_json"])
        run["verification_results_json"] = _loads_json_or_raw(run["verification_results_json"])
        run["next_suggestions_json"] = _loads_json_or_raw(run["next_suggestions_json"])
        run["task_ids_json"] = _loads_json_or_raw(run["task_ids_json"])
        return run
    except Exception as exc:
        raise RuntimeError(f"Failed to query agent run: {run_id}") from exc
    finally:
        if conn is not None:
            conn.close()
...
def update_agent_run(
    ...
    """部分更新 agent_run 记录。仅更新传入的非 None 字段。"""
    conn = None
    try:
        conn = get_conn()
        cursor = conn.cursor()
        ...
        if goal_type is not None:
            set_clauses.append("goal_type = ?")
            params.append(goal_type)
        if plan_steps is not None:
            set_clauses.append("plan_json = ?")
            params.append(json.dumps(plan_steps, ensure_ascii=False))
        if current_step is not None:
            set_clauses.append("current_step = ?")
            params.append(current_step)
        if completed_steps is not None:
            set_clauses.append("completed_steps_json = ?")
            params.append(json.dumps(completed_steps, ensure_ascii=False))
        if pending_steps is not None:
            set_clauses.append("pending_steps_json = ?")
            params.append(json.dumps(pending_steps, ensure_ascii=False))
        if failed_steps is not None:
            set_clauses.append("failed_steps_json = ?")
            params.append(json.dumps(failed_steps, ensure_ascii=False))
        if blockers is not None:
            set_clauses.append("blockers_json = ?")
            params.append(json.dumps(blockers, ensure_ascii=False))
        if next_action is not None:
            set_clauses.append("next_action = ?")
            params.append(next_action)
        if acceptance_criteria is not None:
            set_clauses.append("acceptance_criteria_json = ?")
            params.append(json.dumps(acceptance_criteria, ensure_ascii=False))
        if verification_results is not None:
            set_clauses.append("verification_results_json = ?")
            params.append(json.dumps(verification_results, ensure_ascii=False))
        if replan_count is not None:
            set_clauses.append("replan_count = ?")
            params.append(replan_count)
        if final_report is not None:
            set_clauses.append("final_report = ?")
            params.append(final_report)
        if next_suggestions is not None:
            set_clauses.append("next_suggestions_json = ?")
            params.append(json.dumps(next_suggestions, ensure_ascii=False))
        if task_ids is not None:
            set_clauses.append("task_ids_json = ?")
            params.append(json.dumps(task_ids, ensure_ascii=False))

        if not set_clauses:
            return get_agent_run(run_id, user_id=user_id)

        set_clauses.append("updated_at = CURRENT_TIMESTAMP")
        params.extend([run_id, user_id])

        query = f"UPDATE agent_run SET {', '.join(set_clauses)} WHERE id = ? AND user_id = ?"
        execute(cursor, query, tuple(params))
        conn.commit()
        return get_agent_run(run_id, user_id=user_id)
    except Exception as exc:
        if conn is not None:
            conn.rollback()
        raise RuntimeError(f"Failed to update agent run: {run_id}") from exc
    finally:
        if conn is not None:
            conn.close()
...
def list_agent_runs(
    ...
    try:
        ...
        raise RuntimeError("Failed to list agent runs") from exc
    finally:
        if conn is not None:
            conn.close()
