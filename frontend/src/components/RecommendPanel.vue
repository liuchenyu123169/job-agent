...
import { getMatchScore } from "./utils.js";
...
async function run() {
  ...
  try {
    ...
    await fetchTasks();
  } catch (err) { setMessage(err?.message || "推荐失败", true); }
  finally { loadingMap.recommend = false; }
}
</script>

<template>
  <div class="grid form-grid">
    <label class="field"><span>Top K</span><input v-model="topK" type="number" min="1" max="10" /></label>
    <label class="field"><span>Max Jobs</span><input v-model="maxJobs" type="number" min="1" max="20" /></label>
  </div>
  ...
  <div v-if="result?.items?.length" class="result-list section-top">
    <div v-for="item in result.items" :key="item.job_id" class="result-card">
      <div class="result-card-head">
        ...
        <button class="btn btn-secondary btn-small" @click="applyJobSelection({ id: item.job_id, local_job_id: item.local_job_id, company: item.company, title: item.title }); $emit('close')">选用</button>
      </div>
