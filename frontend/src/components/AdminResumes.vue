  ...
  <div>
    ...
    <div class="table-wrap section-top">
      <table class="task-table">
        <thead><tr><th>ID</th><th>用户</th><th>本地ID</th><th>文件名</th><th>上传时间</th><th></th></tr></thead>
        <tbody>
          <template v-for="r in items" :key="r.id">
            <tr>
              ...
              <td><button class="btn btn-secondary btn-small" @click="toggle(r.id)">{{ expandedId === r.id ? '收起' : '详情' }}</button></td>
            </tr>
            <tr v-if="expandedId === r.id">
              <td colspan="6" style="padding:16px;background:#f8fafc">
                <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px;font-size:12px;color:#64748b">
                  <span>用户: <strong>{{ r.username }}</strong></span>
                  <span>文件: {{ r.file_name }}</span>
                  <span>上传: {{ (r.created_at || '').slice(0, 16) }}</span>
                </div>
                <h5>简历正文</h5>
                <pre class="code-block" style="max-height:400px;white-space:pre-wrap">{{ contentPreview(r.content) }}</pre>
                <button v-if="r.content && r.content.length > 800" class="btn btn-secondary btn-small section-top" @click="alert('完整内容已在下方展开')">展开全文（暂不支持）</button>
              </td>
            </tr>
          </template>
        </tbody>
