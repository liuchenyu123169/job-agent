...
<template>
  <div>
    ...
    <p style="color:#64748b;font-size:12px">按耗时降序，点击展开查看各阶段时间线和 token 用量。</p>
    <div class="table-wrap section-top">
      <table class="task-table">
        <thead><tr><th>ID</th><th>类型</th><th>用户</th><th>简历</th><th>岗位</th><th>总耗时</th><th>spans</th><th>时间</th><th></th></tr></thead>
        <tbody>
          <template v-for="t in items" :key="t.id">
            <tr>
              <td>{{ t.id }}</td><td>{{ t.task_type }}</td><td>{{ t.username }}</td>
              <td>{{ t.resume_id ?? '-' }}</td><td>{{ t.job_id ?? '-' }}</td>
              <td><strong>{{ totalDuration(t.trace_json) || t.total_duration_ms }}ms</strong></td>
