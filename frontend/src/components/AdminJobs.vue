...
async function load() {
  try {
    const r = await api.adminApi.listAllJobs({ page: page.value, page_size: pageSize, username: search.value });
  ...
  <div>
    ...
    <div class="table-wrap section-top">
      <table class="task-table">
        <thead><tr><th>ID</th><th>用户</th><th>本地ID</th><th>公司</th><th>职位</th><th>创建时间</th><th></th></tr></thead>
        <tbody>
          <template v-for="j in items" :key="j.id">
            <tr>
              <td>{{ j.id }}</td><td>{{ j.username }}</td><td>{{ j.local_job_id }}</td>
              <td>{{ j.company }}</td><td>{{ j.title }}</td>
              <td>{{ (j.created_at || '').slice(0, 16) }}</td>
              <td><button class="btn btn-secondary btn-small" @click="toggle(j.id)">{{ expandedId === j.id ? '收起' : '详情' }}</button></td>
            </tr>
            <tr v-if="expandedId === j.id">
              <td colspan="7" style="padding:16px;background:#f8fafc">
                <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px;font-size:12px;color:#64748b">
                  <span>用户: <strong>{{ j.username }}</strong></span>
                  <span>{{ j.company }} — {{ j.title }}</span>
                  <span>创建: {{ (j.created_at || '').slice(0, 16) }}</span>
                </div>
                <h5>岗位 JD</h5>
                <pre class="code-block" style="max-height:400px;white-space:pre-wrap">{{ j.jd_text || '(无 JD 内容)' }}</pre>
              </td>
            </tr>
          </template>
        </tbody>
