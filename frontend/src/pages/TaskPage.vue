...
async function fetchTasks() {
  ...
  try {
    ...
    setMessage(err?.message || "获取任务失败", true);
  } finally {
...
async function fetchDetail(taskId) {
  ...
  try {
    ...
    setMessage(err?.message || "获取详情失败", true);
  } finally {
    loadingMap.taskDetail = false;
  }
}
...
<template>
    ...
    <table class="task-table">
      ...
      <tbody>
        <tr v-for="t in items" :key="t.id">
          ...
          <td>{{ t.created_at }}</td>
          <td>
            <button class="btn btn-secondary btn-small" @click="fetchDetail(t.id)">详情</button>
          </td>
        </tr>
      </tbody>
    </table>
  ...
  <div v-if="selectedTask" class="task-detail section-top">
    ...
    <div class="info-list">
      <div class="info-item">
        <span>类型</span><strong>{{ selectedTask.task_type }}</strong>
      </div>
      <div class="info-item">
        <span>状态</span><strong>{{ selectedTask.status }}</strong>
      </div>
      <div class="info-item">
        <span>简历ID</span><strong>{{ selectedTask.resume_id ?? "-" }}</strong>
      </div>
      <div class="info-item">
        <span>岗位ID</span><strong>{{ selectedTask.job_id ?? "-" }}</strong>
      </div>
    </div>
    <!-- 结构化结果渲染 -->
    <div v-if="parsedOutput" class="result-sections section-top">
      ...
      <template v-if="parsedOutput.analysis?.match_score !== undefined">
        <div class="step-detail">
          ...
          <p v-if="parsedOutput.analysis.match_reason" class="detail-text">
            {{ parsedOutput.analysis.match_reason }}
          </p>
          <div v-if="parsedOutput.analysis.advantages?.length" class="detail-item">
            <h6>优势</h6>
            <ul>
              <li v-for="a in parsedOutput.analysis.advantages" :key="a">{{ a }}</li>
            </ul>
          </div>
          <div v-if="parsedOutput.analysis.weaknesses?.length" class="detail-item">
            <h6>不足</h6>
            <ul>
              <li v-for="w in parsedOutput.analysis.weaknesses" :key="w">{{ w }}</li>
            </ul>
          </div>
          <div v-if="parsedOutput.analysis.suggestions?.length" class="detail-item">
            <h6>建议</h6>
            <ul>
              <li v-for="s in parsedOutput.analysis.suggestions" :key="s">{{ s }}</li>
            </ul>
          </div>
        </div>
      </template>
      ...
      <template v-if="parsedOutput.optimization">
        <div class="step-detail">
          <p v-if="parsedOutput.optimization.summary" class="detail-text">
            {{ parsedOutput.optimization.summary }}
          </p>
          <div v-if="parsedOutput.optimization.skill_keywords?.length" class="detail-item">
            <h6>技能关键词</h6>
            <div class="tag-list">
              <span v-for="k in parsedOutput.optimization.skill_keywords" :key="k" class="tag">{{
                k
              }}</span>
            </div>
          </div>
          <div v-if="parsedOutput.optimization.project_suggestions?.length" class="detail-item">
            <h6>项目建议</h6>
            <ul>
              <li v-for="p in parsedOutput.optimization.project_suggestions" :key="p">{{ p }}</li>
            </ul>
          </div>
          <div
            v-if="parsedOutput.optimization.resume_rewrite_suggestions?.length"
            class="detail-item"
          >
            <h6>简历改写建议</h6>
            <ul>
              <li v-for="r in parsedOutput.optimization.resume_rewrite_suggestions" :key="r">
                {{ r }}
              </li>
            </ul>
          </div>
          <div v-if="parsedOutput.optimization.risk_points?.length" class="detail-item">
            <h6>风险点</h6>
            <ul>
              <li v-for="r in parsedOutput.optimization.risk_points" :key="r">{{ r }}</li>
            </ul>
          </div>
        </div>
      </template>
      ...
      <template v-if="parsedOutput.questions">
        <div class="step-detail">
          <div
            v-for="cat in [
              ['technical_questions', '技术题'],
              ['project_questions', '项目题'],
              ['behavior_questions', '行为题'],
              ['risk_questions', '风险题'],
            ]"
            :key="cat[0]"
          >
            <template v-if="parsedOutput.questions[cat[0]]?.length">
              ...
              <div
                v-for="(q, qi) in parsedOutput.questions[cat[0]]"
                :key="qi"
                class="question-item"
              >
      ...
      <div v-if="parsedOutput.generated_resume" class="step-detail">
        <h6>生成的简历</h6>
        <div
          class="generated-resume-text"
          v-html="parsedOutput.generated_resume.replace(/\n/g, '<br>')"
        ></div>
      </div>
      ...
      <!-- 兜底：无法识别类型 → raw JSON -->
      <pre v-if="!parsedOutput._hasKnownContent" class="code-block">{{
        formatJson(selectedTask.output_json)
      }}</pre>
    </div>
