...
async function saveJob() {
  if (!form.value.title || !form.value.jd_text) {
    setMessage("请填写岗位名称和 JD。", true);
    return;
  }
  loadingMap.jobCreate = true;
  try {
    const resp = await api.jobApi.createJob({
      company: form.value.company || null,
      title: form.value.title,
      jd_text: form.value.jd_text,
    });
    applyJobSelection({
      id: resp.job_id,
      local_job_id: resp.local_job_id,
...
async function fetchDetail() {
  ...
  try {
    ...
    setMessage(err?.message || "查询失败", true);
  } finally {
    loadingMap.jobDetail = false;
  }
}
...
async function refreshList() {
  ...
  try {
    list.value = (await api.jobApi.listJobs()).items || [];
  } catch (err) {
    setMessage(err?.message || "获取列表失败", true);
  } finally {
...
<template>
  <div class="form-stack">
    <label class="field"
      ><span>公司</span><input v-model="form.company" placeholder="如：字节跳动"
    /></label>
    <label class="field"
      ><span>岗位</span><input v-model="form.title" placeholder="如：后端开发工程师"
    /></label>
    <label class="field"
      ><span>岗位 JD</span><textarea v-model="form.jd_text" placeholder="粘贴 JD 内容"></textarea>
    </label>
  ...
  <div class="inline-form">
    ...
    <button class="btn btn-secondary" :disabled="loadingMap.jobDetail" @click="fetchDetail">
      查询
    </button>
  ...
  <div v-if="list.length" class="result-list section-top">
    <div v-for="item in list" :key="item.id" class="result-card">
      <div class="result-card-head">
        <div>
          <h5>#{{ item.local_job_id }} {{ item.company || "?" }} / {{ item.title }}</h5>
        </div>
        <button
          class="btn btn-secondary btn-small"
          @click="
            applyJobSelection(item);
            $emit('close');
          "
        >
          选用
