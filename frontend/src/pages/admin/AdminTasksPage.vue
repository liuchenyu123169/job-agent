  ...
  <div>
    ...
    <div class="inline-form">
      ...
      >
        ...
        <option>MATCH_ANALYZE</option>
        <option>RESUME_OPTIMIZE</option>
        <option>INTERVIEW_QUESTIONS</option>
        <option>JOB_RECOMMEND</option>
        ...
        <tbody>
          <template v-for="t in items" :key="t.id">
            <tr :style="{ background: t.status === 'FAILED' ? '#fef2f2' : '' }">
              ...
              <td>
                <span
                  :style="{ color: t.status === 'FAILED' ? '#dc2626' : '#059669', fontWeight: 600 }"
                  >{{ t.status }}</span
                >
              </td>
              <td>
                <strong>{{ totalDuration(t.trace_json || t.trace) }}ms</strong>
              </td>
              ...
              <td colspan="10" style="padding: 16px; background: #f8fafc">
                <div style="display: block; margin-bottom: 4px; font-size: 12px; color: #334155">
                  <div style="margin-bottom: 2px">
                    <strong>任务 #{{ t.id }}</strong> — {{ t.task_type }}
                  </div>
                  <div style="margin-bottom: 2px">
                    用户: {{ t.username }} | 时间: {{ (t.created_at || "").slice(0, 16) }}
                  </div>
                  <div style="margin-bottom: 2px" v-if="t.resume_id || t.job_id">
                    简历 #{{ t.resume_id ?? "-" }} | 岗位 #{{ t.job_id ?? "-" }}
                  </div>
                  <div style="margin-bottom: 2px">
                    总耗时: <strong>{{ totalDuration(t.trace_json) }}ms</strong> | spans:
                    {{ (t.trace_json || []).length }}
                  </div>
                </div>
                <TaskTrace v-if="t.trace_json?.length" :trace="t.trace_json" />
                <div
                  v-if="!t.trace_json?.length"
                  style="color: #94a3b8; font-size: 12px; margin-bottom: 8px"
                >
                  无链路追踪数据
                </div>
                <div v-if="t.output_json" class="section-top">
                  ...
                  <template v-if="t.output_json.text && typeof t.output_json.text === 'string'">
                    <div class="output-card">
                      <div
                        class="stream-text stream-done"
                        v-html="renderMarkdown(t.output_json.text)"
                      ></div>
                    </div>
                  </template>
                  <pre v-else class="code-block" style="max-height: 300px">{{
                    typeof t.output_json === "string"
                      ? t.output_json
                      : JSON.stringify(t.output_json, null, 2)
                  }}</pre>
                </div>
