  ...
  <div class="admin-users">
    ...
    <div class="table-wrap section-top">
      <table class="task-table">
        <thead><tr><th>ID</th><th>用户名</th><th>管理员</th><th>注册时间</th><th>操作</th><th></th></tr></thead>
        <tbody>
          <template v-for="u in users" :key="u.id">
            <tr>
              ...
              <td><button class="btn btn-secondary btn-small" @click="toggle(u.id)">{{ expandedId === u.id ? '收起' : '详情' }}</button></td>
            </tr>
            <tr v-if="expandedId === u.id">
              <td colspan="6" style="padding:16px;background:#f8fafc">
                <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px;font-size:12px;color:#64748b">
                  <span>用户: <strong>{{ u.username }}</strong></span>
                  <span>注册: {{ (u.created_at || '').slice(0, 10) }}</span>
                  <span>管理员: {{ u.is_admin ? '是' : '否' }}</span>
                </div>
                <div v-if="userStats[u.id]" class="stats-grid">
                  <div class="stat-card clickable" @click="goUserResumes(u.username)"><span class="stat-num">{{ userStats[u.id].resumes ?? '...' }}</span><span class="stat-label">简历</span></div>
                  <div class="stat-card clickable" @click="goUserResumes(u.username)"><span class="stat-num">{{ userStats[u.id].jobs ?? '...' }}</span><span class="stat-label">岗位</span></div>
                  <div class="stat-card clickable" @click="goUserTasks(u.username)"><span class="stat-num">{{ userStats[u.id].tasks ?? '...' }}</span><span class="stat-label">任务</span></div>
                  <div class="stat-card clickable"><span class="stat-num">{{ userStats[u.id].sessions ?? '...' }}</span><span class="stat-label">会话</span></div>
                </div>
...
.stat-card.clickable:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-color: #93c5fd; }
.stat-num { display: block; font-size: 20px; font-weight: 800; color: #2563eb; }
