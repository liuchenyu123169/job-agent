...
function parseTaskIds(session) {
  ...
  try {
    if (session.task_ids_json) {
      const parsed = typeof session.task_ids_json === 'string' ? JSON.parse(session.task_ids_json) : session.task_ids_json;
...
<template>
  <div>
    ...
    <div class="inline-form">
      ...
      <button class="btn btn-secondary btn-small" @click="page=1;load()">搜索</button>
      <span class="page-info">共 {{ total }} 条，第 {{ page }} 页</span>
    </div>
    <div class="table-wrap section-top">
      <table class="task-table">
        <thead><tr><th>ID</th><th>用户</th><th>目标</th><th>状态</th><th>任务</th><th>创建时间</th><th></th></tr></thead>
        <tbody>
          <template v-for="s in items" :key="s.id">
            <tr :style="{ background: s.status === 'ERROR' ? '#fef2f2' : '' }">
              ...
              <td><span :style="{ color: s.status === 'ERROR' ? '#dc2626' : s.status === 'COMPLETED' ? '#059669' : '#64748b', fontWeight: 600 }">{{ s.status }}</span></td>
            ...
            <tr v-if="expandedId === s.id">
              <td colspan="7" style="padding:16px;background:#f8fafc">
                <div style="display:flex;gap:16px;flex-wrap:wrap;margin-bottom:12px;font-size:12px;color:#64748b">
                  <span>目标: <strong>{{ s.goal }}</strong></span>
                  <span v-if="parseTaskIds(s).length">关联任务: {{ parseTaskIds(s).join(', ') }}</span>
                  <span v-if="s.messages_summary">摘要: {{ s.messages_summary }}</span>
                </div>
                <h5>消息记录</h5>
                <div v-if="sessionMsgs[s.id]?.length">
                  <div v-for="(m, i) in sessionMsgs[s.id]" :key="i" style="margin-bottom:8px;font-size:13px;background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:8px 12px">
                    <strong :style="{ color: m.role === 'user' ? '#2563eb' : '#059669' }">{{ m.role === 'user' ? '👤 用户' : '🤖 Copilot' }}</strong>
                    <div style="color:#334155;margin-top:4px;white-space:pre-wrap">{{ (m.content || '').replace('__COPILOT_REPORT__', '').slice(0, 500) }}{{ (m.content || '').length > 500 ? '...' : '' }}</div>
                  </div>
                </div>
...
.inline-form input { width: 180px; height: 32px; border-radius: 8px; border: 1px solid #cbd5e1; padding: 0 8px; font-size: 12px; }
