...
async function uploadResume() {
  ...
  try {
    ...
    applyResumeSelection({
      id: resp.resume_id,
      local_resume_id: resp.local_resume_id,
...
async function refreshList() {
  ...
  try {
    list.value = (await api.resumeApi.listResumes()).items || [];
  } catch (err) {
...
<template>
  <div class="form-stack">
    <label class="field"
      ><span>上传简历文件</span><input type="file" accept=".pdf,.docx,.txt" @change="onFileChange"
    /></label>
  ...
  <div class="inline-form">
    ...
    <button class="btn btn-secondary" :disabled="loadingMap.resumeDetail" @click="fetchDetail">
      查询
    </button>
  ...
  <div v-if="list.length" class="result-list section-top">
    <div v-for="item in list" :key="item.id" class="result-card">
      <div class="result-card-head">
        <div>
          <h5>#{{ item.local_resume_id }} {{ item.file_name }}</h5>
        </div>
        <button
          class="btn btn-secondary btn-small"
          @click="
            applyResumeSelection(item);
            $emit('close');
          "
        >
          选用
